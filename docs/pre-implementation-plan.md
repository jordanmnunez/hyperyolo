...
- **Session storage**: JSON at `~/.config/hyperyolo/sessions.json` (per PRD). Use a lockfile (e.g., `proper-lockfile`) plus atomic temp-write-and-rename to avoid corruption when multiple HyperYOLO instances run.
